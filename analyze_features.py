...
def extract_feature_data(html_file: Path) -> Feature | None:
    ...
    try:
        ...
        steps_section = re.search(r'<section data-steps>.*?</section>', content, re.DOTALL)
        if steps_section:
        ...
        session_match = re.search(r'data-claimed-by-session=["\']?([^"\'>\s]+)', content)
...
def analyze_data(features: list[Feature]) -> dict:
    ...
    for feature in features:
        ...
        # Identify issues
        steps_pct = (feature.steps_completed / feature.steps_total * 100) if feature.steps_total > 0 else 0

        # Abandoned: todo status, 0 steps
        if feature.status == "todo" and feature.steps_completed == 0 and feature.steps_total > 0:
        ...
        elif feature.status == "done" and feature.steps_completed < feature.steps_total and feature.steps_total > 0:
...
def generate_report(features: list[Feature], analysis: dict) -> str:
    ...
    report.append(f"    - Features with agent assigned: {tracked_count} ({tracked_count/analysis['total_features']*100:.1f}%)")
    report.append(f"    - Features WITHOUT agent assigned: {untracked_count} ({untracked_count/analysis['total_features']*100:.1f}%)")
    report.append("")

    if analysis["agent_attribution"]:
        report.append("  Agents who created features:")
        for agent, count in sorted(analysis["agent_attribution"].items(), key=lambda x: -x[1]):
    ...
    if analysis["abandoned_features"]:
        ...
        report.append("   Created but never worked on (status=todo, 0 steps completed):")
        for feature in sorted(analysis["abandoned_features"], key=lambda f: f.title)[:10]:
            agent_info = f"[agent: {feature.agent_assigned}]" if feature.agent_assigned else "[UNTRACKED]"
    ...
    if analysis["partially_done"]:
        ...
        for feature in sorted(analysis["partially_done"], key=lambda f: f.steps_completed/max(f.steps_total, 1), reverse=True)[:10]:
            pct = feature.steps_completed / feature.steps_total * 100 if feature.steps_total > 0 else 0
            report.append(f"   - {feature.id}: {feature.steps_completed}/{feature.steps_total} steps ({pct:.0f}%) - status={feature.status}")
    ...
    if analysis["status_mismatch"]:
        report.append(f"3. STATUS MISMATCH - DONE BUT STEPS INCOMPLETE ({len(analysis['status_mismatch'])}):")
        report.append("   Features marked 'done' but not all steps completed:")
        for feature in sorted(analysis["status_mismatch"], key=lambda f: f.steps_total - f.steps_completed)[:10]:
            missing = feature.steps_total - feature.steps_completed
            report.append(f"   - {feature.id}: {feature.steps_completed}/{feature.steps_total} steps (missing {missing})")
            report.append(f"     Title: {feature.title}")
    ...
    if analysis["untracked_work"]:
        ...
        for feature in sorted(analysis["untracked_work"], key=lambda f: f.steps_completed, reverse=True)[:10]:
            report.append(f"   - {feature.id}: {feature.steps_completed}/{feature.steps_total} steps completed - status={feature.status}")
            report.append(f"     Title: {feature.title}")
    ...
    if analysis["untracked_features"]:
        report.append(f"5. UNTRACKED FEATURES (no agent_assigned) ({len(analysis['untracked_features'])}):")
    ...
    todo_count = analysis['status_distribution'].get('todo', 0)
    in_prog_count = analysis['status_distribution'].get('in-progress', 0) or analysis['status_distribution'].get('in_progress', 0)
    ...
    report.append(f"Primary Issue: {total_issues} features have data integrity problems ({total_issues/analysis['total_features']*100:.1f}%)")
    report.append("")

    if analysis["abandoned_features"]:
        abandoned_pct = len(analysis["abandoned_features"]) / analysis['total_features'] * 100
        report.append(f"1. Abandoned Work: {len(analysis['abandoned_features'])} features in 'todo' status with 0 work ({abandoned_pct:.1f}%)")
    ...
    if analysis["partially_done"]:
        report.append(f"2. Incomplete Tracking: {len(analysis['partially_done'])} features have partial work but status not updated")
        report.append("   Possible causes:")
        report.append("   - Steps being completed but feature status not promoted to 'done'")
    ...
    if analysis["status_mismatch"]:
        report.append(f"3. Status Drift: {len(analysis['status_mismatch'])} features marked 'done' but have incomplete steps")
        report.append("   Possible causes:")
    ...
    if analysis["untracked_features"]:
        report.append(f"4. Missing Agent Attribution: {len(analysis['untracked_features'])} features have no agent_assigned")
        report.append("   Possible causes:")
    ...
    report.append("  1. Partially done features → Mark status as 'in-progress' or 'done' based on step completion %")
    ...
    report.append("     Action: Review each feature - either complete missing steps or downgrade status")
    ...
    report.append("     Action: Update feature creation to capture agent from SDK context")
    report.append("     Impact: Prevents future untracked work")
    report.append("")
    report.append("  5. Implement status sync workflow")
    report.append("     Action: Create hook to auto-update feature status when steps change")
    ...
    report.append("   - Review " + str(len(analysis["status_mismatch"])) + " features marked 'done' with incomplete steps")
    report.append("   - Either complete missing steps or change status")
    report.append("")
    report.append("2. Promote in-progress features")
    report.append("   - " + str(len(analysis["partially_done"])) + " features have partial work")
    ...
    report.append("   - " + str(len(analysis["abandoned_features"])) + " features in 'todo' with no work")
...
if __name__ == "__main__":
    ...
    # Save report
    report_path = Path("/Users/shakes/DevProjects/htmlgraph/FEATURE_INTEGRITY_ANALYSIS.md")
    report_path.write_text(report)
    print(f"\n\nReport saved to: {report_path}")

    # Summary for display
    print("\n\nQUICK STATS:")
    print(f"Total issues found: {len(features) - sum(1 for f in features if f.status == 'done' and f.steps_completed == f.steps_total)}")
