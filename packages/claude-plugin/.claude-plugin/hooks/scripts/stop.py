...
# ///
"""
HtmlGraph Stop Hook with CIGS Integration

Generates comprehensive session summary with:
- Delegation metrics and violations
...
def _bootstrap_pythonpath(project_dir: str) -> None:
    """Bootstrap Python path to load htmlgraph from local dev or venv."""
...
try:
    ...
    print(
        f"Warning: HtmlGraph CIGS not available ({e}). Install with: pip install htmlgraph",
        file=sys.stderr,
    )
    print(json.dumps({}))
    sys.exit(0)
...
def main() -> None:
    ...
    if not cigs_enabled:
        # CIGS not enabled, just output empty response
