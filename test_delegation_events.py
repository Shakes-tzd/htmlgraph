...
def main():
    ...
    ]).set_priority("high").set_status("in_progress").save()
    ...
    ]).set_priority("medium").set_status("in_progress").save()
    ...
    feature3 = sdk.features.create(
        title="Delegation Test Feature 3",
        description="Third test feature with 2 steps"
    ).add_steps([
        "Step 1: Validate delegation events",
        "Step 2: Collect delegation metrics"
    ]).set_priority("low").set_status("in_progress").save()
